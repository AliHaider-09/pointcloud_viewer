...
export const VECTOR3 = {
	...
	wgsl: `
		fn mapping(pointID : u32, attrib : AttributeDescriptor, node : Node, position : vec4f) -> vec4f {
			...
			// Normalize: if any channel > 255, data is true 16-bit, scale all consistently
			...
			// Black points (no RGB data): show intensity as grayscale instead
			var brightness = color.r + color.g + color.b;
			if(brightness < 0.01) {
				if(uniforms.intensityAttIndex != -1) {
					var intAttrib = attributes[uniforms.intensityAttIndex];
					var intOffset = node.numPoints * intAttrib.offset + intAttrib.byteSize * pointID;
					var intValue = f32(readU16(intOffset));
					var intW = (intValue - intAttrib.range_min.x) / (intAttrib.range_max.x - intAttrib.range_min.x);
					intW = clamp(intW, 0.0, 1.0);
					color = vec4f(intW, intW, intW, 1.0);
				} else {
...
export const ELEVATION = {
	...
	wgsl: `fn mapping(pointID : u32, attrib : AttributeDescriptor, node : Node, position : vec4f) -> vec4f {
		...
		// Normalize Z using attribute range (computed from actual point stats min/max Z)
		var w = (worldPos.z - attrib.range_min.x) / (attrib.range_max.x - attrib.range_min.x);
		w = clamp(w, 0.0, 1.0);
