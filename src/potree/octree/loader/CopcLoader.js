...
let activeWorkers_copc = new Map();

export function cancelStaleWorkers_copc(){
	for(let [worker, info] of activeWorkers_copc){
...
export class CopcLoader{
	...
	async loadNode(node){
		...
		try{
			...
			let workerPath = new URL("./DecoderWorker_copc.js", import.meta.url).href;
			let worker = WorkerPool.getWorker(workerPath, {type: "module"});
			let dispatchEpoch = loadEpoch;
			activeWorkers_copc.set(worker, {epoch: dispatchEpoch, workerPath});
			...
			worker.onmessage = (e) => {
				...
				if(data === "cancelled"){
					...
					node.loading = false;
					nodesLoading--;
					activeWorkers_copc.delete(worker);
