...
export async function makePipeline(renderer, args = {}){
	...
	try{
		...
		console.warn("[Pipeline] device lost during pipeline creation, aborting");
		state.stage = "error";
		return;
	}

	pipelinePromise = pipelinePromise.catch(e => {
		console.warn("[Pipeline] pipeline compilation failed (device lost?):", e.message);
		state.stage = "error";
		return null;
	});
