...
fn toQuadPos(vertexID : u32, viewPos : vec4<f32>, node : Node) -> vec4<f32>{
	...
	// QUAD - screen-space pixel-sized quads controlled by pointSize
	var localIndex = vertexID % 6u;

	// Project center to clip space
	var clipPos = uniforms.proj * viewPos;

	// Convert pointSize from pixels to clip-space offset
	// half-size in NDC = pointSize / screen_dim, then * w for clip space
	var hx = uniforms.pointSize / uniforms.screen_width * clipPos.w;
	var hy = uniforms.pointSize / uniforms.screen_height * clipPos.w;

	var outPos = clipPos;

	if(localIndex == 0u){
		outPos.x = clipPos.x - hx;
		outPos.y = clipPos.y - hy;
