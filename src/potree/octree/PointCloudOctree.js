...
export class PointCloudOctree extends SceneNode{
	...
	updateVisibility_additive(camera, renderer){
		...
		let octreeRadius = Math.sqrt(3) * octreeSize / 2;
		let framebufferSize = renderer.getSize();

		// Pre-compute per-frame constants (avoid recalculating per child)
		let fov = math.toRadians(camera.fov);
		let slope = Math.tan(fov / 2);
		let fbHeight = framebufferSize.height;
		let maxLoadQueue = this._maxLoadQueue ?? 5;
		...
		while (priorityQueue.size() > 0) {
			...
			if(!node.loaded){
				...
				// Only queue nodes that are inside the frustum or low-level
				...
				if(inFrustum || node.level <= 2){
					node.weight = weight;
					if(loadQueue.length < maxLoadQueue){
						loadQueue.push(node);
					}
				}
			...
			if(numPoints >= (this.pointBudget ?? Infinity)){
				continue;
			}
			...
			for(let child of node.children){
				...
				// Use child's own bounding box so closer nodes get higher priority
				...
				if(distance - radius < 0){
					// Camera is inside this node — prioritize by closeness
					weight = Number.MAX_VALUE / (1 + distance);
				}

				_vec4.set(
					0.5 * (child.boundingBox.min.x + child.boundingBox.max.x),
					0.5 * (child.boundingBox.min.y + child.boundingBox.max.y),
					0.5 * (child.boundingBox.min.z + child.boundingBox.max.z),
					1.0
				);
		...
		// loadQueue is already in priority order from the BinaryHeap
		...
		this.nodesQueued = loadQueue.length;
	...
	static clearLRU(renderer){
		...
		for(let i = 0; i < 20; i++){
			...
			let tooOld = (lru.newest.timestamp - lru.oldest.timestamp) > 5000;
			let item = lru.oldest;
			let node = item.node;
