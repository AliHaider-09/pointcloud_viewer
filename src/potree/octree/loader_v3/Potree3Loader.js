...
let activeWorkers_v3 = new Map();

export function cancelStaleWorkers_v3(){
	for(let [worker, info] of activeWorkers_v3){
...
// Scale concurrency with available hardware threads, but stay within
...
export class Potree3Loader{
	...
	async loadNodeUnfiltered(node){
		...
		if(numActiveRequests > MAX_ACTIVE_REQUESTS) return;
	...
	async loadNode(node){
		...
		if(WorkerPool.getWorkerCount(workerPath) > hardwareConcurrency)
		...
		try{
			...
			activeWorkers_v3.set(worker, {epoch: dispatchEpoch, workerPath});
			...
			worker.onmessage = (e) => {
				...
				if(data === "cancelled"){
					...
					activeWorkers_v3.delete(worker);
	...
	static async load(url){
		...
		// Pre-warm worker pool based on hardware threads
		let numWorkers = Math.max(navigator.hardwareConcurrency || 4, 4);
		WorkerPool.prewarm(new URL("./DecoderWorker.js", import.meta.url).href, {type: "module"}, numWorkers);
