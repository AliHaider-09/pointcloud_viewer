...
function updateUniforms(octree, octreeState, drawstate, flags){
	...
	{ // UPDATE UNIFORM BUFFER
		...
		let intensityAtt = octree.attributes.attributes.find(a => a.name === "intensity");
		let intensityAttIndex = octree.attributes.attributes.indexOf(intensityAtt);
		intensityAttIndex = Number.isInteger(intensityAttIndex) ? intensityAttIndex : -1;
		uniformsView.setInt32(336, intensityAttIndex, true);
	...
	{ // UPDATE ATTRIBUTES BUFFER
		...
		let set = (index, args) => {
			...
			if(!mapping){
				// Fallback: try to find any mapping whose condition matches this attribute
				for(let m of material.mappings){
					if(m.condition && m.condition(args.settings)){
						mapping = m;
						break;
					}
				}
			...
			attributeView.setUint32(  index * stride + 24,       mapping ? mapping.index : 0, true);
...
async function renderOctree(octree, drawstate, flags){
	...
	for(let i = 0; i < nodes.length; i++){
		...
		// Render all nodes as quads so pointSize controls screen-space size
	...
	{ // DRAW ALL NODES AS QUADS (screen-space sized by pointSize)
		...
		for(let [index, node] of largeNodes){
			...
			if(octree.showBoundingBox === true && node.level > 0){
				...
				// Color-code by octree hierarchy level:
				// red (level 1) → yellow (mid) → green (leaf)
				let maxLevel = 10;
				let t = Math.min((node.level - 1) / (maxLevel - 1), 1.0);
				let r, g;
				if(t < 0.5){
					// red → yellow (increase green)
					r = 255;
					g = Math.round(t * 2.0 * 255);
				}else{
					// yellow → green (decrease red)
					r = Math.round((1.0 - (t - 0.5) * 2.0) * 255);
					g = 255;
				}
				let color = new Vector3(r, g, 0);
