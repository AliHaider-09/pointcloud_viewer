...
export class Renderer{
	...
	async init(){
		...
		if(this.adapter?.info){
			...
			this.gpuName = info.device || info.description || info.architecture || "Unknown GPU";
			console.log(`[Renderer] GPU: ${this.gpuName} (${info.vendor || "unknown vendor"})`);
		}
		...
		this.device.lost.then((info) => {
			console.error(`[Renderer] WebGPU device lost: ${info.message} (reason: ${info.reason})`);
	...
	async readPixels(texture, x, y, width, height){
		...
		// Clamp copy region to texture bounds
